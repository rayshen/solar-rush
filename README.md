...
- The Milky Way is procedural rather than photographic, but its conventional Galactic plane and Hipparcos stars are transformed from Galactic/ICRS coordinates through J2000 equatorial coordinates into the same J2000 ecliptic scene frame used by the planets. This preserves the approximately `60.19°` Galactic-plane/ecliptic-plane angle. The physical dynamical-center anchor uses the observed Sgr A* position (`λ 266.8517°`, `β −5.6077°`) rather than treating the historical Galactic-coordinate origin as identical to Sgr A*. The visible-light model renders a broad warm Sagittarius bulge instead of a bright point at Sgr A*, whose nuclear star field is hidden by foreground dust; bulge density, star clouds, and patchy extinction remain visual approximations.
