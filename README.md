...
- Experience real celestial textures, a J2000-aligned NASA deep-sky map, catalogue stars, orbital paths, and motion trails.
...
- `1P/Halley` uses the smooth JPL#75 osculating Kepler ellipse in visual mode, with one uniform scale for both the orbit guide and comet position; non-linear planet-distance compression is deliberately not applied because it deforms an ellipse. Physical-scale positions use geometric heliocentric state vectors sampled offline from [JPL Horizons](https://ssd.jpl.nasa.gov/horizons/) solution 75 in the IAU76/J2000 ecliptic frame. The 1800–2200 physical dataset includes DE441 planetary perturbations, the SB441-N16 small-perturber model, and the fitted Marsden non-gravitational comet terms. Outside the sampled interval the physical position falls back to the published osculating elements and should be treated as an approximation.
...
- The Solar System sky uses the 8K Milky Way background layer from [NASA SVS Deep Star Maps 2020](https://svs.gsfc.nasa.gov/4851/), generated from Hipparcos-2, Tycho-2, and Gaia DR2 catalogue measurements rather than captured as a single photograph. Its ICRF/J2000 plate carrée map is projected onto a celestial sphere and transformed into the same J2000 ecliptic scene frame used by the planets and catalogue-star layer. This preserves the real sky orientation and the approximately `60.19°` Galactic-plane/ecliptic-plane angle. Bright catalogue stars are rendered separately; the additional drifting particles used during fast travel are an artistic depth cue, not measured stellar motion.
...
Planet and comet positions are suitable for interactive visualization, not spacecraft navigation or professional observation planning. The sampled Halley interpolation and out-of-range fallback, moon propagation, visual scale compression, the brightness treatment of the catalogue-derived sky, the face-on Galaxy reconstruction, drifting background particles, and motion trails remain approximations or artistic interpretations. Texture sources and licensing details are documented in [ATTRIBUTION.md](public/textures/ATTRIBUTION.md).
