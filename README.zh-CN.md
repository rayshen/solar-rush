...
- 使用真实天体纹理、J2000 对齐的 Gaia EDR3 全天空银河观测图、星表恒星、轨道与运动光迹增强空间层次。
...
- 太阳系背景采用 ESA/Gaia EDR3 等距圆柱全天空彩色图，由超过 18 亿颗恒星的实测总亮度和颜色合成。图像的银河坐标与 Hipparcos 亮星层统一经 J2000 赤道坐标转换到行星所用的 J2000 黄道场景坐标，保留约 `60.19°` 的银道面—黄道面夹角。银心亮度集中、尘埃吸收暗带、恒星云和麦哲伦云因此来自巡天数据，而不是程序化密度参数。它是从太阳系观测到的积分全天空投影，并非完整的银河系三维重建。
...
行星位置适用于交互式可视化，不应替代航天导航或专业观测规划。卫星轨道外推、视觉比例压缩、银河俯视重建图和运动光迹仍包含近似或艺术化表达；Gaia 背景是观测得到的全天空积分投影，并非带距离信息的完整三维恒星图。纹理素材的来源和授权信息见 [ATTRIBUTION.md](public/textures/ATTRIBUTION.md)。
