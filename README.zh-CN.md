...
- 使用真实天体纹理、J2000 对齐的 NASA 深空图、星表恒星、轨道与运动光迹增强空间层次。
...
- `1P/Halley` 在视觉模式中直接使用平滑的 JPL#75 瞬时开普勒椭圆，轨道参考线和彗星位置采用同一个统一比例；不再套用会扭曲椭圆的非线性行星距离压缩。真实物理比例模式的位置则使用 [JPL Horizons](https://ssd.jpl.nasa.gov/horizons/) 第 75 号解离线采样的几何日心状态向量，参考系为 IAU76/J2000 黄道。1800–2200 年物理数据包含 DE441 行星扰动、SB441-N16 小天体扰动模型，以及拟合的 Marsden 彗星非引力项；超出采样区间后会退回公开瞬时轨道根数，只应视为近似。
- 主要卫星采用轻量的 J2000 平均轨道参数外推，属于视觉近似，不等同于 Horizons/SPICE 级卫星星历。
- 太阳系天空背景使用 [NASA SVS Deep Star Maps 2020](https://svs.gsfc.nasa.gov/4851/) 的 8K 银河背景层。它由 Hipparcos-2、Tycho-2 和 Gaia DR2 星表观测数据生成，不是单次相机拍摄的全景照片。ICRF/J2000 等距圆柱投影图被映射到天球内侧，并转换到与行星及星表恒星一致的 J2000 黄道场景坐标，从而保留真实天空方向以及约 `60.19°` 的银道面—黄道面夹角。明亮的星表恒星由场景单独渲染；快速航行时额外漂移的星点只是表现纵深的艺术效果，不代表实测恒星运动。
...
行星与彗星位置适用于交互式可视化，不应替代航天导航或专业观测规划。哈雷彗星的采样插值与区间外回退、卫星轨道外推、视觉比例压缩、星表天空的亮度处理、银河俯视重建图、漂移背景星点和运动光迹仍包含近似或艺术化表达。纹理素材的来源和授权信息见 [ATTRIBUTION.md](public/textures/ATTRIBUTION.md)。
