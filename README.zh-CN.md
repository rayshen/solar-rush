...
- 银河不是照片贴图，而是程序化生成；标准银道面与 Hipparcos 恒星从银河/ICRS 坐标经 J2000 赤道坐标转换到与行星一致的 J2000 黄道场景坐标，从而保留约 `60.19°` 的银道面—黄道面夹角。物理动力学中心采用 Sgr A* 的观测位置（黄经 `266.8517°`、黄纬 `−5.6077°`），不再把历史银河坐标原点等同于精确物理银心。可见光模型表现为宽阔、偏暖的人马座核球，而不是在 Sgr A* 处绘制亮点，因为真正的核星场会被前景尘埃遮住；核球密度、恒星云和斑驳消光仍属于视觉近似。
